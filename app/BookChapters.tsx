...
export function BookChapters({children,unlockedThrough=1,initialActive=0,comparisonReady=false,comparisonBusy=false,noClosureJourney=false,onViewComparison}:{children:ReactNode;unlockedThrough?:number;initialActive?:number;comparisonReady?:boolean;comparisonBusy?:boolean;noClosureJourney?:boolean;onViewComparison?:()=>void}){
  ...
  const go=useCallback((index:number,historyMode:"push"|"replace"="push")=>{const next=Math.max(0,Math.min(pages.length-1,index));const sequential=next===active+1&&next<=unlockedThrough;if(next>furthest&&!sequential)return;setActive(next);setFurthest(value=>Math.max(value,next));const id=pages[next]?.props.id;if(id)history[historyMode==="push"?"pushState":"replaceState"](null,"",`#${id}`);window.requestAnimationFrame(()=>{window.scrollTo({top:0,behavior:"auto"});const heading=document.querySelector<HTMLElement>(`.journey-page:not([hidden]) [data-page-heading]`);heading?.focus()})},[pages,active,furthest,unlockedThrough]);

  useEffect(()=>{if(unlockedThrough<furthest)setFurthest(unlockedThrough)},[unlockedThrough,furthest]);
  useEffect(()=>{const selectEvent=(event:Event)=>{const index=pages.findIndex(page=>page.props.id===(event as CustomEvent<string>).detail);if(index>=0)go(index)};const reset=()=>{setActive(0);setFurthest(1);history.replaceState(null,"","#top")};const restore=()=>{const index=pages.findIndex(page=>`#${page.props.id}`===location.hash);if(index>=0&&index<=Math.min(furthest,unlockedThrough))go(index,"replace")};window.addEventListener("six-days-go-step",selectEvent);window.addEventListener("six-days-reset",reset);window.addEventListener("popstate",restore);return()=>{window.removeEventListener("six-days-go-step",selectEvent);window.removeEventListener("six-days-reset",reset);window.removeEventListener("popstate",restore)}},[pages,go,furthest,unlockedThrough]);
