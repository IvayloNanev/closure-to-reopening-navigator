...
const labels=["Start","Closure record","Build comparison","Plan + evidence","Manager summary"];

export function BookChapters({children,unlockedThrough=1,initialActive=0,comparisonReady=false,comparisonBusy=false,onViewComparison}:{children:ReactNode;unlockedThrough?:number;initialActive?:number;comparisonReady?:boolean;comparisonBusy?:boolean;onViewComparison?:()=>void}){
  ...
  const go=useCallback((index:number)=>{const next=Math.max(0,Math.min(pages.length-1,index));const sequential=next===active+1&&next<=unlockedThrough;if(next>furthest&&!sequential)return;setActive(next);setFurthest(value=>Math.max(value,next));const id=pages[next]?.props.id;if(id)history.replaceState(null,"",`#${id}`);window.requestAnimationFrame(()=>window.scrollTo({top:0,behavior:"auto"}))},[pages,active,furthest,unlockedThrough]);

  useEffect(()=>{if(unlockedThrough<furthest)setFurthest(unlockedThrough)},[unlockedThrough,furthest]);
  useEffect(()=>{const selectEvent=(event:Event)=>{const index=pages.findIndex(page=>page.props.id===(event as CustomEvent<string>).detail);if(index>=0)go(index)};const reset=()=>{setActive(0);setFurthest(1);history.replaceState(null,"","#top")};window.addEventListener("six-days-go-step",selectEvent);window.addEventListener("six-days-reset",reset);return()=>{window.removeEventListener("six-days-go-step",selectEvent);window.removeEventListener("six-days-reset",reset)}},[pages,go]);
  useEffect(()=>{window.dispatchEvent(new CustomEvent("six-days-journey-state",{detail:{active,furthest}}))},[active,furthest]);
  ...
  return <div className="guided-journey">
    ...
    {pages.map((page,index)=><div className="journey-page" key={page.props.id??index} hidden={index!==active}>{cloneElement(page,{className:`${page.props.className??""} guided-card is-visible`})}{index>0&&<div className="page-arrows"><button type="button" className="back-arrow" onClick={()=>go(index-1)}>← <span>{index===1?"Start":`Step ${index-1}`}</span></button>{page.props.id==="compare"?<div className="forward-wrap"><button type="button" className="forward-arrow view-comparison-nav" disabled={!comparisonReady||comparisonBusy} onClick={onViewComparison}><span>{comparisonBusy?"Opening comparison…":"View comparison"}</span> {comparisonBusy?"✓":"→"}</button>{!comparisonReady&&<small>Select an option and complete its required settings</small>}</div>:index<pages.length-1?<div className="forward-wrap"><button type="button" className="forward-arrow" disabled={index+1>unlockedThrough||index+1>furthest+1} onClick={()=>go(index+1)}><span>{`Step ${index+1} · ${labels[index+1]}`}</span> →</button>{index+1>unlockedThrough&&<small>{blockedMessage}</small>}</div>:<button type="button" className="forward-arrow" onClick={()=>window.dispatchEvent(new Event("six-days-reset"))}><span>Start another journey</span> ↻</button>}</div>}</div>)}
  </div>
}
