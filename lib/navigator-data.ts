...
async function loadNavigatorData(){
  ...
  for(let offset=0;;offset+=limit){
    ...
    const response=await fetch(`${ENDPOINT}?${params}`,{headers:{Accept:"application/json"},next:{revalidate:21600},signal:AbortSignal.timeout(10000)});if(!response.ok)throw new Error(`NYC Open Data returned ${response.status}`);
